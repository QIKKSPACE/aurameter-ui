...
export default function StickerBottomSheet({ visible, onClose, onStickerSelect,onlinkselect }) {
...
const [linkError, setLinkError] = useState("");
 ...
 {
    id: "link",
    icon: <Icon name="link" size={24} color="#fff" />,
    label: "Add Link",
  ...
  <View style={styles.linkContainer}>
    ...
    <View style={styles.linkInputWrapper}>
      <TextInput
        ...
        onChangeText={(text) => {
          setLinkUrl(text);
          setLinkError(validateLink(text));
        }}
    ...
    {/* CTA */}
    <TouchableOpacity
      disabled={!!linkError || !linkUrl}
      style={[
        styles.linkButton,
        (!!linkError || !linkUrl) && { opacity: 0.4 },
      ]}
      onPress={() => {
        onlinkselect({
          url: linkUrl,
          text: linkUrl.replace("https://", ""),
        });
        setLinkUrl("");
        setMode("stickers");
