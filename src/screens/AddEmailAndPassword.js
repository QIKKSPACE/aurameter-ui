...
const AddEmailAndPassword = ({ route, navigation }) => {
  ...
  const handleSignup = async () => {
    ...
    if (!cleanEmail || !password || !rePassword) {
      showError("All fields are required");
      return;
    }

    // 2. Reject Emojis (using Unicode block matcher)
    const emojiRegex = /\p{Extended_Pictographic}/u;
    if (emojiRegex.test(cleanEmail) || emojiRegex.test(password)) {
      showError("Emojis are not allowed in email or password");
      return;
    }
    ...
    try {
      const response = await fetch("https://api.aurameter.in/auth/signup", {
        ...
        body: JSON.stringify({ username, email: cleanEmail, password, deviceId }),
      });
        ...
        {error ? (
          ...
          >
            <Icon
              ...
              style={{ marginRight: 8 }}
            />
            <AppText variant="caption" style={{ color: theme.text.primary, flex: 1 }}>
              {error}
            </AppText>
          </Animated.View>
        ) : null}
        ...
        >
          ...
          >
            <View style={styles.container}>
              ...
              {/* Email Field */}
              <View style={[styles.inputContainer, { backgroundColor: theme.components.card, borderColor: theme.components.border }]}>
